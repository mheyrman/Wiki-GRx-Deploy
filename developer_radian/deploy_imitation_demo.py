...
class Controller:
    def __init__(self, policy_file_path: str) -> None:
        ...
        self.m_key = None
        self.m_val = self.ref_motions["repeat.pt"] 
        self.m_index = 0

        self.r_motion = self.ref_motions["repeat.pt"]
        self.r_index = 0
    ...
    def _input_loop(self):
        ...
        while self.alive:
            ...
            self.motion_done.wait()

            try:
                cmd = input("motion> ").strip()
            except (EOFError, KeyboardInterrupt):
                print("Shutting down...")
                self.alive = False
                return
            ...
            if cmd in self.ref_motions:
                # Mark the system as BUSY until motion finishes
                self.motion_done.clear()

                print(f"PLAYING MOTION: {cmd}")
                self.m_index = 0
                self.m_key = cmd
                self.m_val = self.ref_motions[cmd]
            else:
                print("Unknown key.")
